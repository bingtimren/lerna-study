...
const fs = require('fs')
const configsDir = path.join(__dirname, "pre-configs")
function list() {
...
program
    .usage('[options] globs...')
    .option('-c, --pre-config <config>', 'choose a pre-config, default to "typescript"', 'typescript')
...
// prepare command
const CMD = `yarn eslint ${options.fix?'--fix':''} --no-error-on-unmatched-pattern -c ${configFile} ${program.args.join(" ")}`
try {
